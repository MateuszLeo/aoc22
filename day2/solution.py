...
left_points = {"A": ROCK, "B": PAPER, "C": SCISSORS}
...
def get_points(l: str, r: str) -> Optional[Tuple[int, int]]:
    ...
    if outcome := next(
        (win_table for win_table in win_tables if win_table == (l, r)), None
    ):
        return outcome_score["lost"] + lsp, outcome_score["won"] + rsc


def part1(left: str, right: str) -> int:
...
def part2(left: str, right: str) -> int:
